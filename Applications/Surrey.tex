...
\begin{center}
\textbf{AI \& Software Engineer | C\# \& Python}
\end{center}

AI-focused Software Engineer with 4+ years of experience building data-driven systems in fintech and SaaS, and an MSc in Advanced Computer Science (Distinction) with a dissertation on AI-based behavioural analysis and summarisation. Experienced designing and deploying \textbf{Python} and \textbf{C\#/.NET} services that integrate \textbf{machine learning} and \textbf{large language models} (Azure OpenAI, BART) into real-world products, including end-to-end pipelines for data collection, preprocessing, model inference, and monitoring. Skilled in \textbf{SQL} and \textbf{Power BI} for analytics and visualisation, and comfortable explaining complex technical concepts to non-technical stakeholders. Highly motivated to apply AI, computer vision, and data analytics to improve animal welfare and support evidence-based decision-making in conservation settings.
...
\begin{resume_list}
    \item \textbf{AI \& Data Science:} Applied machine learning and AI (transformer-based NLP with BART and Azure OpenAI), text analytics, feature engineering, model evaluation, prompt design, and building end-to-end inference pipelines in production.
    \item \textbf{Programming:} Python and C\# for data processing, backend services, and ML-powered applications; JavaScript/TypeScript and Angular for data-driven frontends and visualisation.
    \item \textbf{Data \& BI:} SQL Server, Azure SQL Database, MySQL, PostgreSQL, Oracle, MongoDB; data modelling, querying and performance tuning; dashboards and reports with \textbf{Power BI}.
    \item \textbf{Cloud \& MLOps:} Microsoft Azure (Functions, Service Bus, App Services, Key Vault, Blob Storage, Redis Cache, Logic Apps, OpenAI Service, Speech Services, SQL Database, AD B2C), Azure DevOps, AWS (EC2, Lambda), CI/CD pipelines.
    \item \textbf{Software Engineering:} RESTful APIs, microservices, ASP.NET Core, Entity Framework Core/EF 8, Clean Architecture, SOLID principles, background job queues (RabbitMQ), secure development and authentication/authorisation.
    \item \textbf{Testing \& Communication:} Unit testing, integration testing, Selenium, SoapUI, Swagger, JWT, Postman, automated workflows; technical documentation, reporting, and stakeholder communication.
\end{resume_list}
...
\begin{resume_list}
    \item Architect and develop AI-enabled SaaS backends for Scholarbot using \textbf{.NET 8} and \textbf{ASP.NET Core Minimal APIs}, implementing microservices with \textbf{Entity Framework Core} code-first migrations for scalable multi-tenant data persistence.
    \item Build comprehensive \textbf{Power Apps} canvas and model-driven applications integrated with \textbf{.NET Core} REST APIs for Mentore.app no-code website builder, enabling non-technical users to configure AI-driven course templates and manage content without code changes.
    ...
    \item Implement enterprise workflow automation using \textbf{Microsoft Power Automate} and \textbf{Azure Logic Apps}, orchestrating document processing pipelines, notifications, and data flows across SaaS products.
    \item Integrate \textbf{Azure OpenAI Service} with \textbf{.NET Core} services for intelligent document analysis and summarisation in Booksquare GPT, processing files through \textbf{Azure Functions} background job queues and surfacing insights to users.
    \item Produce technical documentation, API specifications, and internal guides to support knowledge transfer and adoption of AI capabilities across the organisation.
\end{resume_list}
...
\begin{itemize}
    ...
    \item \textbf{MyNewsDigest (React, Flask, Python, MySQL, BART, NLP):} Designed and implemented an AI system for modelling and summarising political discussion on social media using modern NLP techniques. Built an end-to-end Python pipeline for collecting, cleaning, and preprocessing Twitter data, performing topic modelling (TF-IDF, LDA), and generating abstractive summaries with BART. Evaluated model performance as part of an MSc dissertation and exposed results via a web dashboard to make complex behavioural patterns more accessible.
