...
Full-Stack .NET Developer with 4+ years of experience designing, building, and debugging scalable, secure web applications, SaaS ERP platforms, and payment solutions. Strong back-end focus in \textbf{C\#/.NET Core} (ASP.NET MVC, ASP.NET Core Web API, Minimal APIs, Entity Framework, LINQ) with front-end experience in \textbf{TypeScript/Angular}. Proficient in \textbf{MS SQL Server}, REST API design, SOLID principles, and clean code practices, deploying cloud-hosted solutions on \textbf{Azure} with CI/CD and Git-based workflows. Experienced working in Agile teams to translate business requirements into reliable, testable software that supports high-availability, data-sensitive systems.
...
\begin{resume_list}
    \item \textbf{Backend (.NET):} C\#, .NET Core / .NET 8, ASP.NET MVC, ASP.NET Core Web API \& Minimal APIs, RESTful services, microservices, Entity Framework Core / EF 8, LINQ, Dapper, ADO.NET, Multithreading, Clean Architecture, SOLID, clean code practices.
    \item \textbf{Frontend:} TypeScript/JavaScript, Angular 16, Tailwind CSS, Bootstrap 4; collaborating with front-end developers to integrate user-facing elements with server-side logic.
    \item \textbf{Cloud \& DevOps:} Microsoft Azure (App Services, Functions, Service Bus, Key Vault, Blob Storage, Redis Cache, Logic Apps, SQL Database, AD B2C), Azure DevOps, AWS (EC2, Lambda), CI/CD pipelines, Git, Bitbucket; familiarity with Docker-based deployments.
    \item \textbf{Databases:} SQL Server, Azure SQL Database, Oracle, MySQL, PostgreSQL, MongoDB; schema design, stored procedures, performance tuning, and integration across services.
    \item \textbf{APIs \& Integration:} WebAPI (REST/SOAP), API design for extensibility and integration, SOAP/REST, RabbitMQ, ISO-8583 messaging, background job queues, Stripe API, payment gateways.
    \item \textbf{Testing \& Quality:} Unit testing, automated regression testing, Selenium, SoapUI, Swagger, JWT, Postman; experience in Agile/Scrum environments with TDD-influenced practices and code reviews.
\end{resume_list}
...
\begin{resume_list}
    \item Engineered and debugged SaaS product backends for the Scholarbot homework application using \textbf{.NET 8} and \textbf{ASP.NET Core Minimal APIs}, implementing microservices with \textbf{Entity Framework Core} code-first migrations for scalable multi-tenant data persistence.
    \item Build comprehensive \textbf{Power Apps} canvas and model-driven applications integrated with \textbf{.NET Core} REST APIs for Mentore.app no-code website builder, enabling course creators to configure AI-driven course templates and manage content without code changes, reducing configuration time by 40\%.
    \item Developed secure RESTful APIs using \textbf{ASP.NET Core Web API} and \textbf{Dapper ORM} for high-performance data access, implementing authentication with \textbf{Azure AD B2C}, input validation, and API versioning strategies.
    \item Implemented workflow automation using \textbf{Microsoft Power Automate} and \textbf{Azure Logic Apps}, orchestrating document processing pipelines and notification systems across SaaS products.
    \item Investigated and resolved production issues by analysing logs and metrics across services, eliminating bottlenecks and improving response times for end-users.
    \item Design admin portals using \textbf{Power Apps} and \textbf{Dynamics 365} with custom connectors to \textbf{Azure App Services}, providing self-service management for subscription tiers, customer data, and tenant-specific configurations.
...
\begin{resume_list}
    \item Implemented transaction validation and encryption modules in the SOA-based Etisalat Payment Gateway using \textbf{C\#}, \textbf{Entity Framework}, and \textbf{Azure Key Vault}, strengthening payment security and reducing processing errors by 40\% for Gulf merchants.
    ...
    \item Designed and optimized RESTful \textbf{.NET Web APIs} for wallet registration and session management by merging redundant flows into a single step, simplifying integration and reducing code duplication.
    ...
    \item Enhanced API performance and reliability through \textbf{EF Core} query tuning, \textbf{LINQ} optimisation, and selective use of \textbf{Dapper ORM}, resulting in faster response times.
...
\begin{resume_list}
    ...
    \item Debugged complex application issues across services and database layers (e.g., inconsistent billing calculations, delayed approvals), resolving defects and improving overall system stability.
