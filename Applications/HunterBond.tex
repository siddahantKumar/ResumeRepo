...
Software Engineer with commercial experience developing clean, maintainable \textbf{C\#/.NET} applications and \textbf{REST APIs}. Strong foundations in \textbf{object-oriented programming}, SOLID principles, and clean architecture, with hands-on experience in \textbf{Entity Framework Core}, \textbf{LINQ}, and \textbf{SQL Server}. Comfortable collaborating in cross-functional teams, participating in code reviews, debugging complex issues, and writing unit/integration tests to deliver reliable backend services.
...
\begin{resume_list}
    \item \textbf{C\#/.NET:} C\#, .NET Core/.NET 8, ASP.NET MVC, ASP.NET Core Web API \& Minimal APIs, OOP, SOLID, Clean Architecture, LINQ, Entity Framework Core/EF 8, Dapper, ADO.NET
    \item \textbf{APIs \& Backend:} REST API design, authentication (JWT, Azure AD B2C), background jobs, debugging, performance tuning
    \item \textbf{Databases:} SQL Server, Azure SQL Database, Oracle, MySQL, PostgreSQL, MongoDB; schema design, stored procedures, query optimisation
    \item \textbf{Testing \& Quality:} Unit/integration testing (\textbf{xUnit}, \textbf{Moq}), Swagger/OpenAPI, Postman, automated regression (Azure DevOps), code reviews
    \item \textbf{Tools \& Cloud:} Git, Bitbucket, Azure (App Services, Functions, Key Vault, Service Bus, Blob Storage), CI/CD pipelines; Angular/TypeScript (working with front-end teams)
\end{resume_list}
...
\begin{resume_list}
    \item Coded and debugged backend services for Scholarbot using \textbf{.NET 8} and \textbf{ASP.NET Core Minimal APIs}, implementing microservices with \textbf{Entity Framework Core} migrations for multi-tenant data persistence.
    ...
    \item Developed RESTful APIs using \textbf{ASP.NET Core Web API} and \textbf{Dapper ORM}, implementing authentication with \textbf{Azure AD B2C} and API versioning.
    \item Wrote \textbf{xUnit} unit tests with \textbf{Moq} for API controllers and services, and supported integration testing to improve regression coverage for releases.
    ...
    \item Contributed to code reviews and improved reliability by investigating bugs and implementing fixes across APIs and workflow automations.
\end{resume_list}
