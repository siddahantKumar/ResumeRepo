...
Software Engineer with 4+ years of experience designing and modernising cloud-first products across fintech payments, automotive ERP, and SaaS platforms. Strong back-end focus in \textbf{C\#/.NET Core} (ASP.NET Core, Web API, microservices) with front-end development in \textbf{TypeScript/Angular}, building scalable, RESTful APIs and clean, maintainable code. Experienced in \textbf{Azure}-hosted solutions, CI/CD with Azure DevOps, SQL/NoSQL databases, and working in Agile Scrum teams alongside testers and product stakeholders. Passionate about engineering robust, testable systems that support real-world finance and mobility use cases.
...
\begin{resume_list}
    \item \textbf{Back-End (C\#/.NET):} C\#, .NET Core / .NET 8, ASP.NET MVC, ASP.NET Core Web API \& Minimal APIs, RESTful services, microservices, Entity Framework Core / EF 8, Dapper, ADO.NET, Multithreading, LINQ, Clean Architecture, SOLID design.
    \item \textbf{Front-End:} TypeScript, Angular 16, JavaScript, Tailwind CSS, Bootstrap 4; experience building SPA-style UIs and component-based front-ends (transferable to React).
    \item \textbf{Cloud \& DevOps:} Microsoft Azure (App Services, Functions, Service Bus, Key Vault, Blob Storage, Redis Cache, Logic Apps, SQL Database, AD B2C), Azure DevOps, AWS (EC2, Lambda), CI/CD pipelines, Git-based workflows.
    \item \textbf{Data Stores:} SQL Server, Azure SQL Database, Oracle, MySQL, PostgreSQL, MongoDB; schema design, stored procedures, performance tuning for transactional and reporting workloads.
    \item \textbf{APIs \& Integration:} WebAPI (REST/SOAP), SOAP/REST integrations, RabbitMQ, ISO-8583 messaging, background job queues, Stripe API, payment gateways.
    \item \textbf{Testing \& Ways of Working:} Unit testing, Selenium, SoapUI, Swagger, JWT, Postman, automated regression via Azure DevOps; Agile Scrum/Kanban, TDD-influenced development, code reviews, CI/CD and DevOps practices.
\end{resume_list}
...
\begin{resume_list}
    \item Engineered and modernised SaaS product backends for the Scholarbot homework platform using \textbf{.NET 8} and \textbf{ASP.NET Core Minimal APIs}, implementing microservices with \textbf{Entity Framework Core} code-first migrations for scalable multi-tenant data persistence.
    ...
    \item Developed RESTful APIs using \textbf{ASP.NET Core Web API} and \textbf{Dapper ORM} for high-performance data access, implementing authentication with \textbf{Azure AD B2C} and API versioning strategies.
...
\begin{resume_list}
    \item Engineered transaction validation and encryption modules in the SOA-based Etisalat Payment Gateway using \textbf{C\#} and \textbf{Azure Key Vault} for secure credential management, strengthening payment security and reducing processing errors by 40\% for Gulf merchants.
    \item Improved transaction success rates by 90\% by conducting scenario-based testing with VISA, MasterCard, and AMEX provider-issued test cards and automating regression testing with \textbf{Azure DevOps} CI/CD pipelines.
    \item Created and optimized RESTful \textbf{.NET Web APIs} for wallet registration and session management by merging redundant flows, simplifying integration and reducing code duplication.
...
\begin{resume_list}
    ...
    \item Developed core ERP modules (Accounting, Inventory, HR, Invoicing, Smart Reporting) with \textbf{TypeScript/Angular} and \textbf{SQL Server}, streamlining operations and improving reliability by 30\%.
