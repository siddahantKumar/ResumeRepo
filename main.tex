...
\begin{resume_list}
    \item Architected and developed SaaS product backend for Scholarbot AI homework application using \textbf{.NET 8} and \textbf{ASP.NET Core Minimal APIs}, implementing microservices with \textbf{Entity Framework Core} code-first migrations for scalable multi-tenant data persistence.
    \item Built comprehensive \textbf{Power Apps} canvas and model-driven applications integrated with \textbf{.NET Core} REST APIs for Mentore.app no-code website builder, enabling course creators to configure AI-driven course templates and manage content without code changes, reducing configuration time by 40\%.
    ...
    \item Implemented enterprise workflow automation using \textbf{Microsoft Power Automate} and \textbf{Azure Logic Apps}, orchestrating document processing pipelines and notification systems across SaaS products.
    \item Designed admin portals using \textbf{Power Apps} and \textbf{Dynamics 365} with custom connectors to \textbf{Azure App Services}, providing self-service management for subscription tiers, customer data, and tenant-specific configurations.
    \item Integrated \textbf{Azure OpenAI Service} with \textbf{.NET Core} services for intelligent document processing in Booksquare GPT, processing files through \textbf{Azure Functions} background job queues, supporting over 1000 daily active users.
\end{resume_list}
